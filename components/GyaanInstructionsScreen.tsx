...
const GyaanInstructionsScreen: React.FC<GyaanInstructionsScreenProps> = ({ onProceed, onBack }) => {
  return (
    <div className="animate-fade-in text-center flex flex-col items-center justify-center p-4">
      ...
      <h2 className="text-5xl font-black mb-4 uppercase tracking-wider text-shadow-fire flex items-baseline justify-center">
        <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-red-500 to-pink-500">Welcome to the Arena</span>
      ...
      <div className="text-left max-w-lg w-full space-y-6 bg-slate-900/40 p-6 rounded-lg border border-slate-700">
        <div>
          <h3 className="text-3xl font-black mb-3 text-cyan-300 uppercase">📜 Rules Simple Hai:</h3>
        ...
        <div>
          <h3 className="text-3xl font-black mb-3 text-red-400 uppercase">⚠️ Warning:</h3>
          <ul className="list-none space-y-2 text-slate-200">
            <li>Ye quiz koi halwa nahi hai.</li>
            <li>Tough, unpredictable, aur thoda aukaat🔥 se bahar ke questions bhi ho sakte hai.</li>
            <li>Btw tumhara examiner ek chaotic AI Expert-Saksham🤖hai isliye questions se safe rahe and satark rahe🛡️</li>
            <li>Vaise sirf asli 🧠 hi yaha survive kar payenge</li>
          </ul>
        </div>
      </div>

      <p className="mt-8 text-2xl text-slate-300 font-extrabold">
        Chalo, prove karo!
      </p>
